...
type GetDecksResponse []DeckDTOResponse

func (h *DecksHTTPHandler) GetDecks(rw http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	decksDomains, err := h.decksService.GetDecks(ctx, userID, limit, offset)
	if err != nil {
		responseHandler.ErrorResponse(
			err,
			"failed to get decks",
	...
	response := GetDecksResponse(deckDTOsFromDomains(decksDomains))
